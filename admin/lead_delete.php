...
if ($arrVals['id'] > 0) {
	...
	if (!$rs) {
		...
		$arrErr[] = 'The record is unavailable or you are not authorized to make changes to the record. Please return to the <a href="leads.php">leads list</a> and try again.';
	} //END: if (!$rs)

	if (getParam('frmSubmit') == 'true' && sizeof($arrErr) == 0) {
		if ($db->Execute('DELETE FROM leads WHERE id = ?', array(intval($arrVals['id']))) && $db->Execute('DELETE FROM carriers_leads WHERE carrier_id = ?', array(intval($arrVals['id'])))) {
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
	<head>
		...
		<meta name="DC.Title" content="Delete Lead" />
...
<!-- END: div id=navigation -->
				<div id="content">
					...
					<div id="column1">
						...
						<p>The lead has been deleted. <a href="leads.php">Return to the lead list.</a></p>
						<?php } elseif ($rs && !$rs->EOF) { ?>
						<p>Are you sure you want to <strong>delete</strong> this lead?</p>
						<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
							...
							<div class="formElementSet formElementSetText">
								<label class="fieldlabel">Lead Name:</label>
								<span class="text"><?php echo htmlspecialchars($rs->fields['customername']); ?></span>
							</div>
							
							<div class="formElementSet formElementSetButton">
								<input style="color: #088C26; float: left; margin-left: 2em; margin-right: 2em;" type="submit" name="submit" value="Delete Lead" />
							...
							<div class="fieldnote">Note: Deleting this lead will also delete all records of any leads sent to it.</div>
						</form>
						<?php } ?>
					</div>
<!-- END: div id=column1 -->
